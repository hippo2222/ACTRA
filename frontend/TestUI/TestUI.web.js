...
const TestUI = (function () {
  ...
  function createRoot(containerElement, task) {
    ...
    const state = core && typeof core.createInitialState === "function"
      ...
      : (function fallbackCreateState() {
        ...
        const mapped = raw.map((q, idx) => ({
          ...
          index: idx,
          text: q.text || q.title || wt('testui_q.question_fallback', 'Вопрос теста'),
          sourceTaskTitle:
    ...
    if (usingCustomLayout) {
      ...
      sidebarTitle.textContent = wt('testui_q.questions_title', 'Вопросы');
    ...
    function renderQuestionHeaderC2() {
      ...
      if (state.questions.length > 0) {
        ...
        const baseMeta = wt('s1.question_n_of_total', 'Вопрос {n} из {total}').replace('{n}', state.currentIndex + 1).replace('{total}', total);
        title.textContent = baseMeta;
        if (state.difficulty != null) {
          meta.textContent = wt('s1.difficulty_label', 'Сложность: {n}').replace('{n}', state.difficulty);
        } else {
          meta.textContent = "";
        }
        currentMeta =
        ...
        title.textContent = wt('testui_q.question_fallback', 'Вопрос теста');
        meta.textContent = wt('testui_q.no_questions', 'В задании нет вопросов');
      }
      ...
      flagLabel.textContent = wt('testui_q.flag_button', 'Пометить');
    ...
    if (!usingCustomLayout && state.questions.length > 1) {
      ...
      legendTitle.textContent = wt('testui_q.legend_title', 'Обозначения');
      ...
      const flagLabel = document.createElement("span");
      flagLabel.textContent = wt('testui_q.legend_flagged', 'Отмечен');

...
class HostedMicrocardsReviewRepository:
    """Postgres-backed storage for user-scoped microcards review documents."""
    ...
    def ensure_schema(self) -> None:
        with postgres_connection(self._dsn) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    CREATE TABLE IF NOT EXISTS actra_hosted_microcards_user_documents (
        ...
        with postgres_connection(self._dsn) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    SELECT payload
                    FROM actra_hosted_microcards_user_documents
    ...
    def write_document(self, user_id: str, doc_kind: str, payload: Any, *, updated_at: str) -> None:
        ...
        with postgres_connection(self._dsn) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO actra_hosted_microcards_user_documents (user_id, doc_kind, payload, updated_at)
    ...
    def import_document_if_absent(self, user_id: str, doc_kind: str, payload: Any, *, updated_at: str) -> None:
        clean_user_id = str(user_id or "").strip()
        clean_doc_kind = str(doc_kind or "").strip()
        if not clean_user_id or not clean_doc_kind:
            return
        with postgres_connection(self._dsn) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    INSERT INTO actra_hosted_microcards_user_documents (user_id, doc_kind, payload, updated_at)
                    VALUES (%s, %s, %s::jsonb, %s)
                    ON CONFLICT (user_id, doc_kind) DO NOTHING
                    """,
                    (
                        clean_user_id,
                        clean_doc_kind,

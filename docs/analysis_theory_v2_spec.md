...
#### Обязательное разделение: контент колоды vs прогресс пользователя

Для режима микрокарточек фиксируется разделение двух слоёв:

- `Microdeck` и `Microcard` = контент колоды (переносимый артефакт);
- `MicrocardReviewState` и `MicrocardReviewEvent` = персональный прогресс пользователя.

Следствия:

- на одном устройстве несколько пользователей могут использовать одну и ту же колоду/карточки (общий локальный контент);
- прогресс повторения не должен смешиваться между пользователями;
- привязка пользователя к доступным колодам может храниться отдельно от содержимого колоды (имя сущности реализации не фиксируется в MVP);
- импорт/экспорт колод в будущем должен касаться контента; экспорт прогресса пользователя рассматривается отдельно.
- незавершённая review-сессия (очередь + позиция) хранится в user-scoped области и может быть продолжена позже (`resume`).
...
#### `MicrocardReviewSession` (resume незавершённой сессии)
...
```json
{
  ...
  "user_id": "user_001",
  "deck_id": "deck_001",
  "card_queue": ["mc_001", "mc_002", "mc_003"],
  "cursor": 1,
  "completed": false,
  "created_at": "2026-02-24T12:00:00Z",
...
- сохранять очередь review-сессии и позицию пользователя;
- позволять `resume` незавершённой сессии после закрытия окна/перезахода;
- не смешиваться между пользователями даже при общей колоде.
...
- создать новую колоду из всего анализа;
- создать новую колоду по `chunk`;
- создать новую колоду по отдельному `unit`;
- отдельно создать новую `pair_match` колоду для `pair_matching` capability;
- добавить карточки из всего анализа в существующую колоду;
- добавить карточки по `chunk` в существующую колоду;
- добавить карточки по `unit` в существующую колоду;
- добавить `pair_match` карточки в существующую колоду.

Примечание по UX:

- выбор между "создать новую" и "добавить в существующую" может быть реализован как отдельные кнопки или через диалог выбора действия/колоды.

### 9.6. Импорт/экспорт микрокарточек (следующий этап, не P9)

Требование к архитектуре P9:

- не проектировать микрокарточки так, чтобы колода была неотделима от прогресса пользователя;
- хранение должно позволять повторное использование одной колоды несколькими пользователями на одном устройстве.

План на следующий этап:

- импорт/экспорт `Microdeck` + `Microcard` как переносимого контента;
- отдельный (опциональный) импорт/экспорт пользовательского прогресса повторения;
- совместимость версий через отдельный `schema_version`/format version для формата экспорта.
...
Примечание по текущему статусу проекта:

- `P1-P8` считаются выполненными ранее;
- текущий рабочий этап: `P9`;
- пункты ниже сохраняются как канонический roadmap и критерии.
...
- можно добавить карточки из анализа в существующую колоду;
- `pair_match` работает;
- состояние повторения сохраняется (прогресс по карточкам);
- возможен `resume` незавершённой review-сессии;
- состояние повторения изолировано по пользователям (даже если колода/карточки общие на одном устройстве);
- отчёт умеет как создавать колоды/поднаборы, так и добавлять поднаборы в существующую колоду.
...
- `shared microdeck content -> user A review` не изменяет `user B` review state/events.
...
### 13.4. Будущий импорт/экспорт микрокарточек (после P9)

Требования на будущее (чтобы не пришлось ломать P9-реализацию):

- формат экспорта колоды отделён от пользовательского прогресса;
- импорт колоды не перезаписывает автоматически прогресс существующих пользователей;
- прогресс (если экспортируется) импортируется отдельной операцией и в user-scoped область;
- конфликт `deck_id` при импорте решается через remap/duplicate strategy, а не через silent overwrite.

## 14. Немедленные следующие шаги (исполнительский порядок)

Примечание:

- это исходный порядок запуска инициативы "с нуля";
- в текущем состоянии проекта `P1-P8` уже выполнены ранее, поэтому фактический следующий этап реализации — `P9`.
